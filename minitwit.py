...
def add_message():
    """Register a new message for the user."""
...
@app.route('/public')
def public_timeline():
    """Displays the latest message of all users."""
...
@app.route('/')
def timeline():
    """Shows a users timeline or if no user in logged in it will
...
def login_check():
    if g.user:
        ...
        return redirect(url_for('non_logged_in'))


@app.route('/public_timeline/g.user')
def logged_in():
    flash('+ Login Check')
